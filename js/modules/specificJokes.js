...
const renderJokesByTerm = async (event) => {
  event.preventDefault();
  const searchTerm = searchJoke.value.trim();
  ...
  if (data.results.length === 0) {
    showErrorMessage('No jokes found for this term');
  } else {
...
const showErrorMessage = (message) => {
  jokeList.innerHTML = `<p>${message}</p>`;
}

const renderJokeList = (jokes) => {
  jokeList.innerHTML = '<ul>' + jokes.map(joke => `<li><a href="product.html?id=${joke.id}" id="${joke.id}">${joke.joke}</a></li>`).join('') + '</ul>';
}
